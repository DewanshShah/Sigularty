...
below — this is new behaviour, not present in earlier versions of this toolkit.
...
**This is the most significant behavioural change in this version of the toolkit.**

`ACCURACY_DROP_THRESHOLD` (in `main.py`) is now a single, global value used in
two distinct ways:

1. **Hyperparameter search selection** (as before) — `find_optimal_epsilon_smart`
   and `find_optimal_pruning_params` use it to decide which searched
   configuration to recommend.
2. **A hard per-technique gate in the actual pipeline** (new) — after EVERY
...
**Why this exists:** previously, only the search algorithms respected this
threshold when *choosing a configuration*. Nothing checked whether the
*final, fully fine-tuned* result — which can differ from search-time estimates,
since search deliberately uses reduced fine-tune epochs and calibration batches
for speed — actually honored that threshold once the real pipeline ran with your
full settings. A technique could pass search-time gating and still come out of
the real pipeline catastrophically degraded with no automatic correction. Now
every technique is independently re-verified in the real pipeline, not just
estimated during search.
...
## Per-Technique Impact Reporting (New)
...
early-abort mechanism — see above), but never set the new
...
- `compression.py`: `apply_structured_pruning()`, `apply_low_rank_
  ...
  `apply_weight_clustering` gained a new `test_loader` parameter, used only
  for this snapshot (it has no early-abort mechanism of its own to share it
  with). `apply_gptq_quantization`'s existing `._gptq_report` gained one
  new key, `total_eligible_layers` (the denominator for `layers_quantized`
  ...
  every gate decision, and gained four new parameters
...
whatever `ACCURACY_DROP_THRESHOLD` happens to be set to. (An earlier design
expressed this as a multiplier `a` applied to `ACCURACY_DROP_THRESHOLD` — e.g.
`a=3.0 × threshold=10.0` gave the same 30pp ceiling — but that meant changing
`ACCURACY_DROP_THRESHOLD` silently moved the abort ceiling too. A direct
value is easier to reason about at a glance and keeps the two knobs
independent.)

**Raised from 30.0 to 40.0** after a real search run showed the cost of
being too aggressive here: a config at a 34.7pp epoch-1 drop got cut off
under the old 30.0pp threshold, never getting the chance to recover the
way a similarly-positioned config (17-23pp epoch-1 drop) did over its
remaining 2 epochs — going from a hopeless-looking 44% down to a healthy
68% training accuracy by epoch 3. 40.0 keeps that kind of borderline case
alive while still cutting off the clearly-hopeless cluster (55pp+ epoch-1
drops in the same run) that showed no realistic path to recovery.
...
# to completion — the old behaviour). Use a very large number (e.g. 1000)
...
just applied the SVD truncation and stopped. Under the new global gating
...
```python
LRF_FINE_TUNE_EPOCHS = 3        # 0 = no fine-tune (LRF's old behaviour)
...
epochs preserves the old no-fine-tune behaviour exactly, for backward
...
## Major-Fine-Tune Epoch-Zero Safety Check (New, Everywhere)
...
The fused Conv2d has these new weights and bias. The BatchNorm layer is replaced
...
The previous implementation used a DFS heuristic to find successor layers. Every
new architecture required new heuristics, and skip connections, SE blocks, and
...
The old detection code looked for the literal substring `'block.3'` in a
module's dotted name — torchvision's MBConv naming for the project conv at
the end of an inverted-residual block. On a real ResNet-50 run, this printed
`Capped — project convs : 0 block.3 layers` on **every single trial** —
ResNet's bottleneck blocks are named `layer4.0.conv3`, `layer4.0.downsample.0`,
etc., which never contains `'block.3'`. The exact protection this toolkit
already had for this exact failure mode (see above) provided zero coverage
for ResNet, ResNeXt, WideResNet, DenseNet, ConvNeXt, or any other residual
family — despite the model registry's own notes claiming pruning "is very
effective on the large conv channels" for ResNet-50 specifically.

**The fix is architecture-agnostic: it uses Torch-Pruning's own dependency
...
4. Every module in a flagged group gets a `pruning_ratio_dict` entry at
   ...
   exactly the same way the old `block.3` layers used to.
...
LRF_FINE_TUNE_EPOCHS    = 3      # NEW — 0 = no recovery fine-tune
LRF_FINE_TUNE_LR        = 0.00001  # NEW
```
...
`LRF_FINE_TUNE_EPOCHS` / `LRF_FINE_TUNE_LR`: New recovery fine-tune (see above).
0 epochs = LRF's old, no-recovery behaviour. Recommended whenever `LRF_EPSILON`
...
**This is now the SOLE fine-tuning method used everywhere in the toolkit** —
Structured Pruning, Low-Rank Factorization (new), Weight Clustering, and the
...
LRF_FINE_TUNE_EPOCHS     = 3      # NEW — KD recovery epochs after LRF (0=skip).
LRF_FINE_TUNE_LR         = 0.00001  # NEW — learning rate for that recovery fine-tune.
...
where that technique ran. This is the new gating behaviour, not a bug — see
...
**Epsilon search reports `acc=0.0%` for every single trial**
On any version of this toolkit before the `nn.MultiheadAttention` safety
check existed, this was the LRF-on-ViT bug — every trial silently failed and
reported a fake 0.0%. This specific case is now caught and prevented before
the search even starts (LRF and the epsilon search are auto-disabled for
`nn.MultiheadAttention` architectures). If you see all-zero accuracy on an
architecture WITHOUT `nn.MultiheadAttention`, it's a genuine new bug worth
investigating — `measure_accuracy` will now `raise` (not silently report
0.0%) if every batch failed, so check the traceback for the actual root cause
rather than assuming this is the same historical bug.
...
Bug reports and ideas are welcome via [GitHub Issues](../../issues).
This project isn't taking external pull requests yet — see
