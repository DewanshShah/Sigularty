...
---------------------------------
  ...
  CQI = accuracy_factor(accuracy, baseline_accuracy, accuracy_drop_threshold)
  ...
  accuracy_factor is a three-piece barrier function of the accuracy drop,
  normalized against accuracy_drop_threshold - see _accuracy_barrier_factor()
  below for the full mechanism. When accuracy_drop_threshold is not supplied,
  this falls back to the legacy (acc/baseline_acc)**w_accuracy ratio for
  backward compatibility with any caller not yet passing the new parameter.
...
def _accuracy_barrier_factor(
    accuracy_drop: float,
    accuracy_drop_threshold: float,
    ceiling: float = 2.0,
    penalty_slope: float = 0.1,
    penalty_steepness: float = 25.0,
) -> float:
    """
    Three-piece accuracy multiplier for the Compression Quality Index.

    Let x = accuracy_drop / accuracy_drop_threshold - the accuracy drop
    normalized against the technique's own hard-gate accuracy budget.
    x=0 means no change; x=1 means the drop exactly equals
    accuracy_drop_threshold (the point at which gate_technique_accuracy
    would revert the technique elsewhere in the pipeline).

    Three pieces, matched continuously at x=0 and x=1 (no gaps, no domain
    restriction anywhere - this function is defined for every real x):

      x < 0   (accuracy IMPROVED over baseline):
        factor = 1 + (ceiling - 1) * (-x) / (1 - x)
        A rational saturating reward. factor -> 1 as x -> 0 (from below),
        factor -> ceiling as x -> -infinity. Bounded - a technique cannot
        earn an unlimited multiplier just by improving accuracy a lot.

      0 <= x < 1   (drop is within the allowed budget):
        factor = 1 - penalty_slope * x
        A gentle, straight-line decline. Using up to 100% of the allowed
        accuracy-drop budget only ever costs `penalty_slope` at most - using
        HALF the budget is only mildly worse than using none of it. This is
        deliberately lenient: the pipeline's own hard gate already decides
        whether a drop within the threshold is acceptable; this factor's
        job is not to re-litigate that, only to gently prefer smaller drops
        among otherwise-similar candidates.

      x >= 1   (drop is AT OR PAST the hard-gate threshold):
        factor = (2 - penalty_slope) - exp(penalty_steepness * (x - 1))
        Continuous with the in-budget piece at x=1 (both give
        1 - penalty_slope there), then falls away UNBOUNDED and
        increasingly steeply as x grows past 1 - `penalty_steepness`
        controls how dramatic that fall is. No finite compression ratio
        can ever compensate for a technique whose accuracy drop is at or
        beyond its own hard-gate threshold: for any fixed penalty_steepness,
        the CQI contribution here shrinks toward -infinity as the drop
        grows, strictly faster than any polynomial size/latency ratio can
        grow to offset it.

    Verified acceptance test (accuracy_drop_threshold=10, defaults above):
      drop=-11  x=-1.1   factor≈1.524
      drop= -1  x=-0.1   factor≈1.091
      drop=  0  x= 0.0   factor= 1.000
      drop=  1  x= 0.1   factor= 0.990
      drop=  5  x= 0.5   factor= 0.950
      drop= 10  x= 1.0   factor= 0.900
      drop= 11  x= 1.1   factor≈-10.283   (gap from drop=10 is ~26x any other
                                            consecutive gap in this sequence)

    Args:
        accuracy_drop:            baseline_accuracy - accuracy, in the same
                                  percentage-point units used everywhere
                                  else in this toolkit (ACCURACY_DROP_THRESHOLD,
                                  gate_technique_accuracy, etc.).
        accuracy_drop_threshold: The technique's own accuracy-drop budget
                                  (pp). Must be > 0.
        ceiling:                  Bounded maximum reward multiplier for
                                  accuracy improvement (x < 0). Default 2.0.
        penalty_slope:             Straight-line decline rate through the
                                  in-budget region (0<=x<1). Default 0.1
                                  (using the full budget costs at most 0.1).
        penalty_steepness:         Growth rate of the unbounded exponential
                                  fall past the threshold (x>=1). Default 25.0.
                                  Higher = more dramatic gap right at the
                                  threshold; does not affect anything for x<1.

    Returns:
        A float accuracy multiplier - see the three pieces above. Never
        raises, never returns NaN/inf for any finite accuracy_drop.
    """
    x = accuracy_drop / accuracy_drop_threshold
    if x < 0.0:
        t = -x
...
def compression_quality_index(
    ...
    accuracy_ceiling: float = 2.0,
    accuracy_penalty_slope: float = 0.1,
    accuracy_penalty_steepness: float = 25.0,
    ...
    accuracy_factor comes from _accuracy_barrier_factor() - see that
    function's docstring for the full three-piece shape - whenever
    accuracy_drop_threshold is supplied (> 0).
    ...
    Args:
        ...
        accuracy_drop_threshold: The technique's own accuracy-drop budget
                                (pp) - same value as ACCURACY_DROP_THRESHOLD
                                / accuracy_drop_threshold used everywhere
                                else in this toolkit. None (default) = use
                                the legacy w_accuracy-exponent formula instead.
        accuracy_ceiling:        _accuracy_barrier_factor's `ceiling` - bounded
                                max reward for accuracy improvement. Only
                                used when accuracy_drop_threshold is supplied.
        accuracy_penalty_slope: _accuracy_barrier_factor's `penalty_slope` -
                                in-budget decline rate. Only used when
                                accuracy_drop_threshold is supplied.
        accuracy_penalty_steepness: _accuracy_barrier_factor's
                                `penalty_steepness` - post-threshold fall
                                rate. Only used when accuracy_drop_threshold
                                is supplied.
        w_accuracy:              Legacy accuracy factor weight (default 1.0).
                                Only used in the fallback path (see above).
    ...
    if accuracy_drop_threshold is not None and accuracy_drop_threshold > 0.0:
        accuracy_drop = baseline_accuracy - accuracy
        acc_factor = _accuracy_barrier_factor(
            accuracy_drop, accuracy_drop_threshold,
            ceiling=accuracy_ceiling,
            penalty_slope=accuracy_penalty_slope,
    ...
    if (latency_ms is not None and baseline_latency_ms is not None
